...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset, id_map = load_and_cache_examples(
...
def main():
    ...
    args, model, model_class, tokenizer, tokenizer_class = base_main(
        parser, logger, MODEL_CLASSES, is_cls=True
    )

    # Training
    if args.do_train:
        train_dataset, id_map = load_and_cache_examples(
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        eval_dataset, id_map = load_and_cache_examples(
            args, eval_task, tokenizer, evaluate=True, predict=True
        )
